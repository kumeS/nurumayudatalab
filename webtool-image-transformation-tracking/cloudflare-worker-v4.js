 ...
 *   - Fixed exports.default function call issues (now uses direct function calls)
 *   - All helper functions are now independent (not methods)
 *   - Compatible with Cloudflare Workers runtime
 *   - Maintains all v3.0 functionality (R2 storage, polling, file extraction)
 *
 * Based on:
 *   - cloudflare-worker-v3.js: R2 storage and file extraction
...
// ============================================================================
// Core Functions (formerly methods in export default object)
// ============================================================================
 ...
 */
async function pollForResult(pollUrl, apiToken, controller, maxAttempts = 20) {
 ...
 * @param {object} responseData - Replicate prediction response
 ...
 * Supports: Nano Banana (array output), TRELLIS v2, video files
 * @param {object} responseData - Replicate prediction response
...
function extractAllFileUrls(responseData) {
    ...
    if (responseData.output) {
        ...
        } else if (typeof responseData.output === 'string' && isValidUrl(responseData.output)) {
...
async function handleCreatePrediction(request, env, ctx, corsHeaders) {
    ...
    if (
        ...
        // Use pollForResult function (fixed: no more exports.default)
    ...
    // Save to R2 if succeeded (fixed: no more exports.default)
    let saved = [];
