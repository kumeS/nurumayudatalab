...
function generatePrintablePDF(report, options = {}) {
  ...
  setTimeout(() => {
    ...
    if (options.onSuccess) {
      options.onSuccess('PDF出力画面を開きました');
    }
...
function generateHTMLReport(report, options = {}) {
  try {
    ...
    // より安全なファイル名生成
    let defaultFilename = `report_${(report.studentName || 'document').replace(/[^\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF\w]/g, '_').substring(0, 20)}_${dateStr}_${timeStr}.html`;
    ...
    const errorMsg = 'HTMLレポートの生成に失敗しました: ' + error.message;
    if (options.onError) {
      options.onError(error);
    } else {
      alert(errorMsg);
