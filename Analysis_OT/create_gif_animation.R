...
# Create animated plot with updated terminology
cat("Generating high-quality animated GIF...\n")

# Calculate axis limits with expanded range (same as v2)
...
anim_plot <- ggplot(anim_data, aes(x = PC1, y = PC2, color = State)) +
  ...
  theme(
    ...
    panel.grid.minor = element_line(color = "grey95", size = 0.4),
...
# Save animation
anim_save("optimal_transport_animation.gif", anim_gif)

cat("✅ High-quality GIF animation created: optimal_transport_animation.gif\n")
