...
*   **Web管理ダッシュボード (`index.html`)**:
    ...
    *   **監視キーワードの管理**: 最大20件まで登録可能、各キーワードごとに取得件数（5〜100件）を指定
...
├── test.sh          # スクレイピング検証用 CLI スクリプト（4サイト対応）
├── watchMate-cli/   # test.shベースのCLIコマンド + MCPブリッジ
...
## ✅ test.sh 実装状況（2026-02-26確認）

`test.sh`は、ローカルで各ECサイトの抽出ロジックを検証するためのCLIとして実装済みです。

- **対応サイト**: `7net` / `hmv` / `animate` / `rakuten` / `all`
- **主要オプション**: `-q, --query`（必須）, `-s, --site`, `-t|-m|--max`, `-p, --product-filter`, `-csvoutput FILE`, `-v, --verbose`, `-h, --help`
- **取得処理**: `curl` のタイムアウト・リトライ付き共通ラッパーで取得失敗時も継続実行
- **販売状況判定**: 商品URLを追加取得して、`販売終了` / `売り切れ` / `予約する` / `販売中` / `判定不可` を表示
- **出力**: サイト別のタイトル・URL・価格・販売状況を整形表示し、最後に合計件数と所要時間を表示

### 実行例

```bash
./test.sh -q "Vジャンプ"
./test.sh -q "ドラゴンボール" --site hmv --max 5
./test.sh -q "ONE PIECE" -p "特装版"
./test.sh -q "Vジャンプ" --site hmv --max 1 -csvoutput results.csv
./test.sh -q "ONE PIECE" -v
```

### 動作要件・注意

- 必須コマンド: `bash`, `curl`, `grep`, `sed`, `tr`, `head`, `wc`
- 推奨コマンド: `python3`（URLエンコードおよび7net抽出で使用）, `iconv`（HMVの文字コード変換）
- サイト側のWAFやHTML構造変更により、取得件数が0件になる場合があります

### Node.js CLI 代替実装（新）

`test.sh` の代替として、`src/runtime/cli.js` を追加しています。

```bash
npm run cli -- -q "Vジャンプ" -s hmv -m 2
npm run cli -- -q "ONE PIECE" -p "特装版" -csvoutput results-node.csv
```

- コアロジック: `src/core/*` に分離
- 実行環境: Node.js 18+
- CSV列: `timestamp,keyword,site,title,price,url,status`

## 🔧 watchMate-cli（test.shベース）

`watchMate-cli/watchMate-cli` は `test.sh` をベースにした実行コマンドです。

```bash
./watchMate-cli/watchMate-cli -q "Vジャンプ"
./watchMate-cli/watchMate-cli -q "ドラゴンボール" -s hmv -m 5
```

ルートの npm script からも実行できます。

```bash
npm run watchmate:cli -- -q "Vジャンプ"
```

### MCP化（AI利用）

`watchMate-cli/mcp/server.mjs` で、`watchmate_search` ツールを提供します。

- stdio: `npm run watchmate:mcp:stdio`
- http: `npm run watchmate:mcp:http`（`POST /mcp`, `GET /health`）

詳細は [watchMate-cli/README.md](watchMate-cli/README.md) を参照してください。
...
3.  **機能**:
    *   **キーワード管理タブ**: 検索キーワードを最大20件まで登録、各キーワードごとに取得件数（5〜100件）、対象サイト、メール通知を設定可能
...
| **DELETE / POST** | `/api/keywords/delete` | キーワードの削除 |
| **PUT / POST** | `/api/keywords/update` | キーワード設定（有効/無効）の更新 |
| **GET** | `/api/data` | 全キーワードの最新検索結果を取得 |
| **GET** | `/api/data/latest` | 特定キーワードの最新検索結果を取得（`keyword` クエリ必須） |
