...
#   ./test.sh -q "ONE PIECE" -p "特装版"
#   ./test.sh -q "Vジャンプ" -csvoutput results.csv
#   ./test.sh --help
# =============================================================================
...
$(echo -e "${BOLD}")オプション:$(echo -e "${RESET}")
  ...
  -p, --product-filter   タイトル部分一致フィルタ（この文字列を含む商品のみ表示）
  -csvoutput FILE        CSV形式で結果を出力（行列形式）
  -v, --verbose          詳細出力モード
  -h, --help             このヘルプを表示
...
$(echo -e "${BOLD}")例:$(echo -e "${RESET}")
  ...
  $0 -q "ONE PIECE" -p "特装版"
  $0 -q "Vジャンプ" -csvoutput results.csv
...
title_matches() {
  local title="$1"
  [[ -z "$PRODUCT_FILTER" ]] && return 0
  printf '%s' "$title" | grep -Fq -- "$PRODUCT_FILTER"
}
...
# CSV初期化
init_csv_output() {
  [[ "$CSV_ENABLED" -ne 1 ]] && return 0
  printf 'timestamp,keyword,site,title,price,url,status\n' > "$CSV_OUTPUT_FILE"
}
...
# 商品ページ本文から販売状況を判定
detect_sales_status_from_file() {
  local html_file="$1"

  local text
  text=$(LC_ALL=C tr '\r\n' '  ' < "$html_file" 2>/dev/null \
    | LC_ALL=C sed 's/<[^>]*>/ /g' 2>/dev/null \
    | LC_ALL=C sed 's/&nbsp;/ /g; s/&amp;/\&/g; s/&lt;/</g; s/&gt;/>/g; s/&quot;/"/g' 2>/dev/null \
    | LC_ALL=C tr -s ' ' 2>/dev/null)
  ...
  if printf '%s' "$text" | grep -qiE '販売終了|販売終了しました|取扱終了|取り扱い終了|終売|販売を終了'; then
    echo "販売終了"
    return 0
  fi

  if printf '%s' "$text" | grep -qiE '売り切れ|売切れ|在庫切れ|品切れ|在庫なし|在庫がありません|SOLD[[:space:]]*OUT|現在ご購入いただけません|ご注文できない商品'; then
    echo "売り切れ"
    return 0
  fi

  if printf '%s' "$text" | grep -qiE '予約する|予約受付中|ご予約受付中|予約受付|予約商品|予約注文'; then
    echo "予約する"
    return 0
  fi

  if printf '%s' "$text" | grep -qiE '販売中|購入する|カートに入れる|買い物かごに入れる|今すぐ購入|注文する|ご注文可能|在庫あり'; then
    echo "販売中"
    return 0
  fi
...
fetch_product_status() {
  ...
  local detail_file="${TMPDIR_BASE}/${site_key}_detail_${index}.html"

  fetch_url "$item_url" "$detail_file" >/dev/null
  if (( $? != 0 )); then
    echo "判定不可"
    return 0
  fi
  ...
  local file_size
  file_size=$(wc -c < "$detail_file" 2>/dev/null | tr -d ' ')
  if (( file_size < 800 )); then
    echo "判定不可"
    return 0
  fi

  detect_sales_status_from_file "$detail_file"
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -p|--product-filter) PRODUCT_FILTER="$2"; shift 2 ;;
    -csvoutput|--csvoutput) CSV_OUTPUT_FILE="$2"; CSV_ENABLED=1; shift 2 ;;
...
if [[ "$CSV_ENABLED" -eq 1 && -z "$CSV_OUTPUT_FILE" ]]; then
  log_error "CSV出力ファイルを指定してください (-csvoutput FILE)"
  echo ""
  usage
fi
...
if [[ -n "$PRODUCT_FILTER" ]]; then
  echo -e "  ${CYAN}タイトル絞込:${RESET} ${PRODUCT_FILTER}"
fi
if [[ "$CSV_ENABLED" -eq 1 ]]; then
  echo -e "  ${CYAN}CSV出力:${RESET}   ${CSV_OUTPUT_FILE}"
fi
echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo ""
...
search_7net() {
  ...
  while IFS=$'\t' read -r item_title item_url item_price; do
    ...
    title_matches "$item_title" || continue
    ...
    local sale_status
    sale_status=$(fetch_product_status "7net" "$count" "$item_url")
    local csv_price="${item_price:-}"
    [[ "$csv_price" == "-" ]] && csv_price=""
    echo -e "  ${GREEN}${count}.${RESET} ${BOLD}${item_title}${RESET}${price_str} ${CYAN}[${sale_status}]${RESET}"
    echo -e "     ${DIM}${item_url}${RESET}"
    append_csv_row "7net" "$item_title" "$csv_price" "$item_url" "$sale_status"
  done < <(
    python3 -c "
...
search_hmv() {
  ...
  for (( i = 0; i < ${#titles_h[@]} && i < MAX_RESULTS; i++ )); do
    title_matches "${titles_h[$i]}" || continue
    (( count >= MAX_RESULTS )) && break
    (( count++ ))
    local price_str=""
    if (( i < ${#prices_h[@]} )); then
    ...
    local sale_status
    sale_status=$(fetch_product_status "hmv" "$count" "${urls_h[$i]}")
    local csv_price=""
    if (( i < ${#prices_h[@]} )); then
      csv_price="${prices_h[$i]}"
    fi
    echo -e "  ${GREEN}${count}.${RESET} ${BOLD}${titles_h[$i]}${RESET}${price_str} ${CYAN}[${sale_status}]${RESET}"
    echo -e "     ${DIM}${urls_h[$i]}${RESET}"
    append_csv_row "hmv" "${titles_h[$i]}" "$csv_price" "${urls_h[$i]}" "$sale_status"
  done
...
search_animate() {
  ...
  for (( i = 0; i < ${#titles_a[@]} && i < MAX_RESULTS; i++ )); do
    title_matches "${titles_a[$i]}" || continue
    (( count >= MAX_RESULTS )) && break
    (( count++ ))
    local price_str=""
    if (( i < ${#prices_a[@]} )); then
    ...
    local sale_status
    sale_status=$(fetch_product_status "animate" "$count" "${urls_a[$i]}")
    local csv_price=""
    if (( i < ${#prices_a[@]} )); then
      csv_price="${prices_a[$i]}"
    fi
    echo -e "  ${GREEN}${count}.${RESET} ${BOLD}${titles_a[$i]}${RESET}${price_str} ${CYAN}[${sale_status}]${RESET}"
    echo -e "     ${DIM}${urls_a[$i]}${RESET}"
    append_csv_row "animate" "${titles_a[$i]}" "$csv_price" "${urls_a[$i]}" "$sale_status"
  done
...
search_rakuten() {
  ...
  for (( i = 0; i < ${#titles_r[@]} && i < MAX_RESULTS; i++ )); do
    title_matches "${titles_r[$i]}" || continue
    (( count >= MAX_RESULTS )) && break
    (( count++ ))
    local price_str=""
    if (( i < ${#prices_r[@]} )); then
    ...
    local sale_status
    sale_status=$(fetch_product_status "rakuten" "$count" "${urls_r[$i]}")
    local csv_price=""
    if (( i < ${#prices_r[@]} )); then
      csv_price="${prices_r[$i]}"
    fi
    echo -e "  ${GREEN}${count}.${RESET} ${BOLD}${titles_r[$i]}${RESET}${price_str} ${CYAN}[${sale_status}]${RESET}"
    echo -e "     ${DIM}${urls_r[$i]}${RESET}"
    append_csv_row "rakuten" "${titles_r[$i]}" "$csv_price" "${urls_r[$i]}" "$sale_status"
  done
...
if [[ "$CSV_ENABLED" -eq 1 ]]; then
  echo -e "  ${CYAN}CSV保存先:${RESET} ${CSV_OUTPUT_FILE}"
fi
echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo ""
